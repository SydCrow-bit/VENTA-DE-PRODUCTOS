from app import create_app
from app.extensions import db
from app.models import User, Category, Product

app = create_app()

def seed_database():
    with app.app_context():
        # Seccion Usuarios
        users_data = [
            {"username": "admin", "password": "12345678", "role": "admin"},
            {"username": "user", "password": "12345678", "role": "user"}
        ]

        for data in users_data:
            user = User.query.filter_by(username=data["username"]).first()
            if user:
                user.role = data["role"]
                user.set_password(data["password"])
            else:
                new_user = User(username=data["username"], role=data["role"])
                new_user.set_password(data["password"])
                db.session.add(new_user)
        
        db.session.commit()

        # Seccion Categorias
        categories_data = ["Laptops", "Smartphones", "Audio", "Accesorios", "Televisores"]
        category_map = {}

        for cat_name in categories_data:
            category = Category.query.filter_by(nombre=cat_name).first()
            if not category:
                category = Category(nombre=cat_name)
                db.session.add(category)
                db.session.commit()
            category_map[cat_name] = category.id

        # Seccion Productos con Imagenes Reales
        products_data = [
            {
                "nombre": "Apple MacBook Pro 14\"", 
                "descripcion": "Chip M3 Pro, 18GB RAM, 512GB SSD.", 
                "precio": 1999.99, "stock": 15, "cat": "Laptops",
                "img": "https://images.unsplash.com/photo-1517336714731-489689fd1ca8?q=80&w=800"
            },
            {
                "nombre": "Dell XPS 15", 
                "descripcion": "Intel Core i7, 16GB RAM, 1TB SSD, RTX 4050.", 
                "precio": 1499.00, "stock": 10, "cat": "Laptops",
                "img": "https://images.unsplash.com/photo-1593642632823-8f785ba67e45?q=80&w=800"
            },
            {
                "nombre": "iPhone 15 Pro", 
                "descripcion": "Apple A17 Pro, 256GB, Titanio Natural.", 
                "precio": 1099.00, "stock": 25, "cat": "Smartphones",
                "img": "https://cdsassets.apple.com/live/7WUAS350/images/tech-specs/iphone_15_pro.png"
            },
            {
                "nombre": "Samsung Galaxy S24 Ultra", 
                "descripcion": "Snapdragon 8 Gen 3, 512GB, IA integrada.", 
                "precio": 1299.99, "stock": 20, "cat": "Smartphones",
                "img": "https://images.unsplash.com/photo-1610945415295-d9bbf067e59c?q=80&w=800"
            },
            {
                "nombre": "Sony WH-1000XM5", 
                "descripcion": "Auriculares inalámbricos con cancelación de ruido activa.", 
                "precio": 398.00, "stock": 30, "cat": "Audio",
                "img": "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?q=80&w=800"
            },
            {
                "nombre": "Apple AirPods Pro 2", 
                "descripcion": "Auriculares in-ear con USB-C y cancelación de ruido.", 
                "precio": 249.00, "stock": 40, "cat": "Audio",
                "img": "https://tecnopolis.com.bo/cdn/shop/files/airp-2.png?crop=center&height=900&v=1757016376&width=720"
            },
            {
                "nombre": "Logitech MX Master 3S", 
                "descripcion": "Ratón inalámbrico ergonómico, sensor 8K DPI.", 
                "precio": 99.99, "stock": 35, "cat": "Accesorios",
                "img": "https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?q=80&w=800"
            },
            {
                "nombre": "Keychron K2", 
                "descripcion": "Teclado mecánico inalámbrico formato 75%.", 
                "precio": 79.00, "stock": 20, "cat": "Accesorios",
                "img": "https://images.unsplash.com/photo-1511467687858-23d96c32e4ae?q=80&w=800"
            },
            {
                "nombre": "LG OLED C3 55\"", 
                "descripcion": "Smart TV 4K, 120Hz, ideal para gaming.", 
                "precio": 1296.00, "stock": 7, "cat": "Televisores",
                "img": "https://images.unsplash.com/photo-1593359677879-a4bb92f829d1?q=80&w=800"
            }
        ]

        # Borramos productos existentes para evitar conflictos con la nueva estructura
        Product.query.delete()

        for prod_data in products_data:
            product = Product(
                nombre=prod_data["nombre"],
                descripcion=prod_data["descripcion"],
                precio=prod_data["precio"],
                stock=prod_data["stock"],
                imagen=prod_data["img"],
                category_id=category_map[prod_data["cat"]]
            )
            db.session.add(product)

        db.session.commit()
        print("Seeding con imagenes completado.")

if __name__ == "__main__":
    seed_database()